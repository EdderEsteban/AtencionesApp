...
public class PacientesController : Controller
{
    ...
    public async Task<IActionResult> Index(string? q)
    {
        ...
        if (pacientes.Any())
        {
            ...
            var ultimasEnf = await _context.AtencionesEnfermeria
                .Where(a => ids.Contains(a.PacienteId))
                .GroupBy(a => a.PacienteId)
                .Select(g => new { Id = g.Key, Fecha = g.Max(a => a.Fecha) })
                .ToListAsync();
            var ultimasOdo = await _context.AtencionesOdontologia
                .Where(a => ids.Contains(a.PacienteId))
                .GroupBy(a => a.PacienteId)
                .Select(g => new { Id = g.Key, Fecha = g.Max(a => a.Fecha) })
                .ToListAsync();
            ViewBag.UltimasAtenciones = ids.ToDictionary(
                id => id,
                id => {
                    var e = ultimasEnf.FirstOrDefault(x => x.Id == id)?.Fecha;
                    var o = ultimasOdo.FirstOrDefault(x => x.Id == id)?.Fecha;
                    if (e == null) return o;
                    if (o == null) return e;
                    return e > o ? e : o;
                });
    ...
    // GET /Pacientes/Buscar?q=xxx  — devuelve JSON para búsqueda en formularios
